...
def test_render_tempfs_ok(report_data, account_factory, report_factory):
    ...
    j2_file = renderer.render(data, path_to_output_file)

    assert '"row_0_col_0",' in open(j2_file).read()
    assert ', "row_1_col_1"' in open(j2_file).read()
    assert j2_file == f'{path_to_output_file}.csv'
